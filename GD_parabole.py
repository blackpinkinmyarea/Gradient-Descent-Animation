...
def check_gradientient(x):
	...
	g_gradient = gradient(x)
	if np.linalg.norm(g-g_gradient) > 1e-5:
		print("WARNING: CHECK gradientIENT FUNCTION!")
...
def gradient_descent(x_init, learning_rate, iteration):
	...
	for i in range(iteration):
		...
		if np.linalg.norm(gradient(x_new))/m < 0.5: # when to stop GD
			break
		x_list.append(x_new)

	return x_list

def update_for_parabole(i):
...
check_gradientient(x_init)

# Run gradientient descent
iteration = 70
...
line_ani = animation.FuncAnimation(fig1, update_for_parabole, iters, interval=50, blit=True)

# legend for plot
plt.legend(('Value in each GD iteration', 'Solution by formular', 'Inital value for GD'), loc=(0.52, 0.78))
ltext = plt.gca().get_legend().get_texts()

# title
plt.title("gradientient Descent Animation")
